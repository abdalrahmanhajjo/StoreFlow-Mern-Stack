...
export function Cart() {
  ...
  const onSelectCustomer = (c: { id: string; name: string; phone?: string; points: number }) =>
    setCustomer({
      id: c.id,
      name: c.name,
      phone: c.phone ?? '',
    ...
    <>
          ...
          {customer && (
            ...
            >
              ...
              >
                ...
                <div
                  className="mono"
                  title={customer.phone || 'No phone number'}
                  style={{
                    fontSize: 11.5,
                    color: 'var(--ink-faint)',
                    fontWeight: 600,
        ...
        <div style={{ overflow: 'auto', minHeight: 0 }}>
          {empty ? (
            ...
            <div>
              ...
              >
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-soft)', letterSpacing: '.04em', textTransform: 'uppercase' }}>
                  Product
                </span>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-soft)', letterSpacing: '.04em', textTransform: 'uppercase', textAlign: 'center', minWidth: 40 }}>
                  Qty
                </span>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-soft)', letterSpacing: '.04em', textTransform: 'uppercase', textAlign: 'right', minWidth: 60 }}>
                  Price
                </span>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-soft)', letterSpacing: '.04em', textTransform: 'uppercase', textAlign: 'right', minWidth: 70 }}>
                  Total
                </span>
              ...
              {items.map((i) => (
                <div
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = 'var(--card-raise)';
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, minWidth: 40, justifyContent: 'center' }}>
                    <button type="button" onClick={() => changeQty(i.id, -1)} aria-label="Decrease" style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 15, width: 20, height: 20, borderRadius: 4, color: 'var(--ink-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                      −
                    </button>

                    <span className="mono" style={{ fontSize: 13, color: 'var(--ink)', fontWeight: 700, minWidth: 20, textAlign: 'center' }}>
                      {i.qty}
                    </span>

                    <button type="button" onClick={() => changeQty(i.id, 1)} aria-label="Increase" style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 15, width: 20, height: 20, borderRadius: 4, color: 'var(--ink-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                      +
                    </button>
                  </div>

                  <div className="mono" style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', minWidth: 60, textAlign: 'right' }}>
                    {money(i.price)}
                  </div>

                  <div className="mono" style={{ fontSize: 13, fontWeight: 800, color: 'var(--ink)', minWidth: 70, textAlign: 'right' }}>
                    {money(i.price * i.qty)}
                  </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                {((discountMode === 'percent' ? discountRate : discountFixed) > 0) && (
              ...
              <span style={{ fontSize: 12, transition: 'transform .2s', display: 'inline-block', transform: discountOpen ? 'rotate(180deg)' : 'none' }}>
                ▾
              </span>
            </button>
              ...
              <div style={{ background: 'var(--card)', borderRadius: 10, border: '1px solid var(--line)', padding: '12px 14px 14px', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 1 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--ink-faint)' }}>
                    cap {discountMode === 'percent' ? `${Math.round(maxPctForRole(role) * 100)}%` : money(maxFixedForRole(role))}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                  <div style={{ display: 'flex', background: 'var(--paper-dim)', borderRadius: 7, overflow: 'hidden', fontSize: 11.5, fontWeight: 700, flexShrink: 0 }}>
                    <button type="button" onClick={() => setDiscountMode('percent')} style={{ padding: '5px 13px', border: 'none', cursor: 'pointer', background: discountMode === 'percent' ? 'var(--ink)' : 'transparent', color: discountMode === 'percent' ? 'var(--card)' : 'var(--ink-soft)', transition: 'all .12s' }}>
                      %
                    </button>
                    <button type="button" onClick={() => setDiscountMode('fixed')} style={{ padding: '5px 13px', border: 'none', cursor: 'pointer', background: discountMode === 'fixed' ? 'var(--ink)' : 'transparent', color: discountMode === 'fixed' ? 'var(--card)' : 'var(--ink-soft)', transition: 'all .12s' }}>
                      $
                    </button>
                  </div>
                  ...
                  {discountMode === 'percent' ? (
                    <>
                      ...
                      <span className="mono" style={{ fontSize: 14, fontWeight: 800, color: 'var(--ink)', minWidth: 36, textAlign: 'right' }}>
                    ...
                    <>
                      ...
                      <span className="mono" style={{ fontSize: 14, fontWeight: 800, color: 'var(--ink)', minWidth: 48, textAlign: 'right' }}>
                        {money(discountFixed)}
                      </span>
                    </>
                  )}
                  ...
                  {(discountMode === 'percent' && discountRate > 0) || (discountMode === 'fixed' && discountFixed > 0) ? (
                    <button type="button" onClick={() => (discountMode === 'percent' ? setDiscountRate(0, role) : setDiscountFixed(0, role))} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 12, color: 'var(--ink-faint)', flexShrink: 0, padding: 4 }}>
                      ✕
                    </button>
                  ) : null}
                </div>
                ...
                {(discountMode === 'percent' ? discountRate > 0 : discountFixed > 0) && (
                  <div style={{ marginTop: 8, fontSize: 11.5, color: 'var(--green)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span>You save</span>
                    <span className="mono" style={{ fontWeight: 800, fontSize: 12 }}>
                      {money(t.discount)}
                    </span>
                  </div>
                )}
            ...
            {customer && (
              <div style={{ fontSize: 11, color: 'var(--green)', fontWeight: 700, textAlign: 'center', marginTop: 10 }}>
            ...
            {parked.map((p) => (
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', background: 'var(--paper)', borderRadius: 'var(--radius)', border: '1px solid var(--line)' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  <button type="button" onClick={() => { onResume(p.id); setParkedOpen(false); }} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--blue-border)', background: 'var(--blue-soft)', color: 'var(--blue-deep)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    Resume
                  </button>
                  <button type="button" onClick={() => removeParked(p.id)} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid var(--line)', background: 'var(--card)', color: 'var(--ink-faint)', fontSize: 12, cursor: 'pointer' }}>
                    ✕
                  </button>
                </div>
              </div>
      ...
      <Modal open={!!confirmPay} onClose={() => setConfirmPay(null)}>
        <div style={{ textAlign: 'center', padding: '6px 0 2px' }}>
          ...
          {customer && (
            <div style={{ fontSize: 11, color: 'var(--green)', fontWeight: 600, marginBottom: 12 }}>
              ★ {customer.name.split(' ')[0]} earns {t.pointsEarned} pts
            </div>
          )}

          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={() => setConfirmPay(null)} style={{ flex: 1, padding: '11px 0', borderRadius: 9, border: '1px solid var(--line)', background: 'var(--card)', color: 'var(--ink-soft)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              Cancel
            </button>
