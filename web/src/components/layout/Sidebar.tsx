  ...
  return (
    <>
      <style>{`
        ...
        .sf-sidebar:hover { scrollbar-color: var(--line) transparent; }
        .sf-sidebar::-webkit-scrollbar { width: 4px; }
        .sf-sidebar::-webkit-scrollbar-thumb { background: transparent; border-radius: 4px; }
        .sf-sidebar:hover::-webkit-scrollbar-thumb { background: var(--line); }
        ...
        .sf-nav-item::before {
          ...
          border-radius: 0 3px 3px 0; background: var(--blue); transform: scaleX(0);
          transition: transform .2s ease;
        }
        .sf-nav-item.active::before { transform: scaleX(1); }
        .sf-nav-item:hover { background: var(--paper-dim); }
        .sf-nav-item.active:hover { background: var(--blue-soft); }
      `}</style>
      <aside style={{
        width: 244, flexShrink: 0, background: 'var(--card)',
        borderRight: '1px solid var(--line)', padding: '20px 14px 16px',
          ...
          {groups.map((g) => (
            <div key={g} style={{ marginBottom: 4 }}>
              <div style={{
                ...
                color: 'var(--ink-faint)', padding: '14px 10px 5px', fontWeight: 700,
              }}>{g}</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12.5, lineHeight: 1.35, minWidth: 0 }}>
            <div style={{ color: 'var(--ink)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name}</div>
