...
export function Cart() {
  ...
  return (
    <>
      <div style={{ background: '#fff', color: 'var(--ink)', display: 'grid', gridTemplateRows: 'auto auto 1fr auto', borderLeft: '1px solid var(--line)', height: '100%', overflow: 'hidden' }}>
        {/* Header */}
        ...
        <div style={{ padding: '12px 20px 14px', borderBottom: '1px solid var(--line)' }}>
          ...
          {customer && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
              ...
              <button type="button" onClick={onRedeem} disabled={customer.points < POINTS_BLOCK} style={{ padding: '5px 14px', borderRadius: 7, border: '1px solid var(--blue-border)', background: redeeming ? 'var(--blue)' : 'var(--blue-soft)', color: redeeming ? '#fff' : 'var(--blue-deep)', fontSize: 12, fontWeight: 700, cursor: customer.points < POINTS_BLOCK ? 'not-allowed' : 'pointer', opacity: customer.points < POINTS_BLOCK ? 0.5 : 1, transition: 'all .12s' }}>
        ...
        <div style={{ overflow: 'auto', minHeight: 0 }}>
          {empty ? (
            ...
            <div>
              {/* Table Header */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto auto', gap: 8, padding: '12px 20px', borderBottom: '1px solid var(--line)', background: '#f8fafc' }}>
        ...
        {/* Totals + checkout */}
        {!empty && (
            <div style={{ padding: '12px 20px 14px', borderTop: '1px solid var(--line)', background: '#f8fafc' }}>
              {/* Discount */}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  {((discountMode === 'percent' ? discountRate : discountFixed) > 0) && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#2563eb' }} />}
                  Discount
              ...
              {discountOpen && (
                <div style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--line)', padding: '12px 14px 14px', marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                    <div style={{ display: 'flex', background: '#f1f5f9', borderRadius: 7, overflow: 'hidden', fontSize: 11.5, fontWeight: 700, flexShrink: 0 }}>
                      <button type="button" onClick={() => setDiscountMode('percent')} style={{ padding: '5px 13px', border: 'none', cursor: 'pointer', background: discountMode === 'percent' ? '#0f172a' : 'transparent', color: discountMode === 'percent' ? '#fff' : 'var(--ink-soft)', transition: 'all .12s' }}>%</button>
                      <button type="button" onClick={() => setDiscountMode('fixed')} style={{ padding: '5px 13px', border: 'none', cursor: 'pointer', background: discountMode === 'fixed' ? '#0f172a' : 'transparent', color: discountMode === 'fixed' ? '#fff' : 'var(--ink-soft)', transition: 'all .12s' }}>$</button>
                    </div>
                    {discountMode === 'percent' ? (
                      <>
                        <input type="range" min={0} max={maxPctForRole(role) * 100} value={discountRate * 100} onChange={(e) => setDiscountRate(Number(e.target.value) / 100, role)} style={{ flex: 1, accentColor: '#2563eb' }} aria-label="Discount percent" />
                      ...
                      <>
                        <input type="range" min={0} max={maxFixedForRole(role)} value={discountFixed} onChange={(e) => setDiscountFixed(Number(e.target.value), role)} style={{ flex: 1, accentColor: '#2563eb' }} aria-label="Discount amount" />
              ...
              {/* Summary Table */}
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--line)', padding: '10px 14px', marginBottom: 12 }}>
              ...
              {/* Payment Button */}
              <button type="button" onClick={() => { setPay(payMethod); setConfirmPay(payMethod); }} style={{ width: '100%', padding: '16px 20px', borderRadius: 10, border: 'none', background: 'linear-gradient(180deg,#3b82f6,#2563eb)', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(37,99,235,.3)', transition: 'all .12s' }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 4px 12px rgba(37,99,235,.4)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(37,99,235,.3)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
              ...
              <div style={{ display: 'flex', gap: 7, marginTop: 10 }}>
                <button type="button" onClick={onHold} style={{ flex: 1, background: '#fff', color: 'var(--ink-soft)', border: '1px solid var(--line)', padding: 10, borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, transition: 'all .1s' }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--paper)'; e.currentTarget.style.borderColor = '#cbd5e1'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = 'var(--line)'; }}
                >⏸ Hold</button>
                <button type="button" onClick={() => { setPay('Cash'); setConfirmPay('Cash'); }} style={{ flex: 1, background: '#fff', color: 'var(--ink-soft)', border: '1px solid var(--line)', padding: 10, borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, transition: 'all .1s' }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = '#fefce8'; e.currentTarget.style.borderColor = '#fde68a'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = 'var(--line)'; }}
                >💵 Cash</button>
                <button type="button" onClick={() => { setPay('Card'); setConfirmPay('Card'); }} style={{ flex: 1, background: '#fff', color: 'var(--ink-soft)', border: '1px solid var(--line)', padding: 10, borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, transition: 'all .1s' }}
      ...
      <Modal open={payOpen} onClose={onCancelPay}>
        <div style={{ background: '#1e293b', borderRadius: 14, padding: 0, overflow: 'hidden', margin: -22 }}>
          {/* Terminal screen */}
          ...
          {payStep === 0 && (
            <div style={{ padding: '28px 24px 24px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#fff', fontSize: 13, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', marginBottom: 4 }}>Insert or tap card</div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '14px 18px', marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, color: '#fff', fontFamily: 'monospace' }}>{money(t.total)}</div>
              </div>
              <button type="button" onClick={startProcessing} style={{ width: '100%', padding: '13px 0', borderRadius: 9, border: 'none', background: 'linear-gradient(180deg,#3b82f6,#2563eb)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Process payment</button>
          ...
          {payStep === 1 && (
            <div style={{ padding: '36px 24px 28px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#fff', fontSize: 14, fontWeight: 600, marginTop: 16 }}>Authorizing transaction…</div>
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginTop: 4 }}>Please wait while the payment is processed</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: '#fff', fontFamily: 'monospace', marginTop: 16 }}>{money(t.total)}</div>
            </div>
          )}

          {payStep === 2 && (
            <div style={{ padding: '36px 24px 28px', textAlign: 'center' }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', fontSize: 26 }}>✓</div>
              <div style={{ color: '#22c55e', fontSize: 15, fontWeight: 700, letterSpacing: '.04em' }}>APPROVED</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: '#fff', fontFamily: 'monospace', margin: '10px 0' }}>{money(t.total)}</div>
            ...
            {parked.map((p) => (
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', background: 'var(--paper)', borderRadius: 'var(--radius)', border: '1px solid var(--line)' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button type="button" onClick={() => removeParked(p.id)} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid var(--line)', background: '#fff', color: 'var(--ink-faint)', fontSize: 12, cursor: 'pointer' }}>✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Modal open={!!confirmPay} onClose={() => setConfirmPay(null)}>
        <div style={{ textAlign: 'center', padding: '6px 0 2px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={() => setConfirmPay(null)} style={{ flex: 1, padding: '11px 0', borderRadius: 9, border: '1px solid var(--line)', background: '#fff', color: 'var(--ink-soft)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
            <button type="button" onClick={proceedPay} style={{ flex: 2, padding: '11px 0', borderRadius: 9, border: 'none', background: confirmPay === 'Cash' ? 'linear-gradient(180deg,#f59e0b,#d97706)' : 'linear-gradient(180deg,#3b82f6,#2563eb)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
