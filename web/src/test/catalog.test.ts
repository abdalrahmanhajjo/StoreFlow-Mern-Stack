...
describe('products store (SF-601)', () => {
  it('derives stock status from reorder point', () => {
    expect(statusFor({ id: 'x', name: '', sku: '', price: 1, cost: 0, stock: 0, reorderPoint: 5, emoji: '', image: '', category: '' })).toBe('Out of stock');
    expect(statusFor({ id: 'x', name: '', sku: '', price: 1, cost: 0, stock: 3, reorderPoint: 5, emoji: '', image: '', category: '' })).toBe('Low stock');
    expect(statusFor({ id: 'x', name: '', sku: '', price: 1, cost: 0, stock: 20, reorderPoint: 5, emoji: '', image: '', category: '' })).toBe('In stock');
  });

  it('rejects duplicate SKU on create', () => {
    const { create } = useProducts.getState();
    const ok = create({ name: 'New', sku: 'ZZZ1', price: 1, cost: 0, stock: 4, reorderPoint: 2, emoji: '🆕', image: '', category: 'Bakery' });
    expect(ok.ok).toBe(true);
    const dup = create({ name: 'Dup', sku: 'ZZZ1', price: 2, cost: 1, stock: 1, reorderPoint: 1, emoji: '🔁', image: '', category: 'Bakery' });
